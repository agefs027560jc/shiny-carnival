...
for v in {0,2}; do
  ...
  echo -e "$title\n" > stats/.pid
  time ./build/deptran_server -f config/test_toy.yml -f config/${c}c1s${r}r1p.yml -f config/rw.yml -f config/concurrent_${cc}.yml -P localhost -v $v -d $duration | tee -a stats/.pid | grep "tid of leader is "
